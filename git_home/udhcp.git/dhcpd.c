...
#ifdef SATELLITE_MODEL_NAME
void read_satellite_name(char (*sate_table)[10])
{
		char *result = NULL;
		int num = 0;
		char my_table[100] = {'\0'};
		snprintf(my_table,100,"%s",SATELLITE_MODEL_NAME);
		result = strtok(my_table,",");
		while(result != NULL){
			strcpy(sate_table[num],result);
			result = strtok(NULL,",");
			num++;
		}
}
void check_satellite_allconfig(struct dhcpOfferedAddr *lease,char (*sate_table)[10])
{
		int i;
		for(i=0;strcmp(sate_table[i],"");i++){
			if(strcmp(lease->hostname,sate_table[i]) == 0){
			char cmd[256]="", hyt_mac[32]="";
			ether_etoa(&lease->chaddr, hyt_mac);
			snprintf(cmd, 256, "/usr/sbin/check_satellite_allconfig %s &", hyt_mac);
			system(cmd);
			break;
			}
		}
...
#ifdef SATELLITE_MODEL_NAME
		char satellite_table[20][10] = {'\0'};
		read_satellite_name(satellite_table);
#endif
		...
		case DHCPDISCOVER:
 		...
 		case DHCPREQUEST:
			...
			if(lease && lease->hostname) {
				check_satellite_allconfig(lease,satellite_table);
			}
#endif
			break;
		case DHCPDECLINE:
