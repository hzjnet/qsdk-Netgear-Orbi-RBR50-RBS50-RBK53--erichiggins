...
#define ARP_FILE	"/tmp/netscan/attach_device_new"
#ifdef HAVE_IGNORE_TABLE
#define ARLO_LIST	"/tmp/netscan/arlo_list"
#endif
#define SATELLITE_NUM_FILE	"/tmp/netscan/satellite_num_new"
#define ATTACHED_DEVICE_NUM_FILE	"/tmp/netscan/attached_device_num_new"
...
#define UA_FILE        "/tmp/device_tables/UA_String_Match_Table.txt"
#define INDEX_FILE    "/tmp/device_tables/device_type_table.json"
#define LOCAL_DEVICE_FILE	"/tmp/device_tables/local_device_table"
#define LOCK_FILE	"/var/run/netscan.pid"
#define LOG_DIR          "/var/log/netscan/"
#define DEFAULT_DEVICE_TYPE 24
...
typedef enum attached_type {
	BASE_ATT,		//devices attached to base
	SATELLITE_ATT,	//devices attached to satellite
	BASE_ATT_S,		//satellites
	NONETYPE_ATT
} AttachedType;
...
#endif
	char satellite_2g_name[32];
	char satellite_5g_name[32];
	char satellite_wired_name[32];
};

#ifdef HAVE_IGNORE_TABLE
struct ignore_table
{
	struct ignore_table *next;
	struct in_addr ip;
	uint8 mac[ETH_ALEN];
};
#endif
...
extern int init_interface_info(void);

extern void reset_arp_table();
extern void scan_arp_table(int force_soap_flag);
...
//extern int get_connection_type(uint8 *mac);
//extern int get_interface_info();
extern void get_model_name(uint8 *mac, char *model);
extern int vendor_filter(char *name);
extern struct vendor vendor_table[];
extern cJSON *getJSONcode(char *filename);
extern char *get_wild_character(char *sku);
extern int invalid_wild(char *ua_t, char *sku);
extern void arp_request(const char *dst_ip_addr, const unsigned char *dst_mac_addr);
extern char *translation_index_and_type(char *string, int flag);
extern void print_log(char *file, int line, const char *fmt, ...);
#ifdef HAVE_IGNORE_TABLE
extern void update_ignore_table(void);
#endif
