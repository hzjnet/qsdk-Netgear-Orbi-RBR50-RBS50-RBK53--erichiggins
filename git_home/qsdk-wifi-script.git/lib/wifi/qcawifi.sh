...
portal_acl_redirect_rules() {
    ...
    if [ "$product" = "SRR60"  -o "$product" = "SRS60" -o "$product" = "RBS50Y" ]; then
...
add_lan_restricted_guest_rule()
{
    ...
    ebtables -A "$Chain" -p "$ETH_P_IPv6" --ip6-dst "$lan_ipv6addr" -j DROP
    ...
    ebtables -A "$Chain" -p "$ETH_P_IP" --ip-dst "$brsubnet" -j DROP
    ebtables -A "$Chain" -p "$ETH_P_IPv6" --ip6-dst "$ipv6subnet" -j DROP
    ...
    ebtables -A "$Chain" -p "$ETH_P_IPv6" --ip6-src "$ipv6subnet" -j DROP
    ebtables -A "$Chain" -p "$ETH_P_IP" --ip-dst 224.0.0.0/4 -j DROP
    ebtables -A "$Chain" -p "$ETH_P_IP" --ip-dst 255.255.255.255 -j DROP
    ebtables -A "$Chain" -p "$ETH_P_IPv6" --ip6-dst FF00::/8 -j DROP
    ebtables -A "$Chain" -d 01:00:00:00:00:00/01:00:00:00:00:00 -j DROP
...
enable_qcawifi() {
	...
	for vif in $vifs; do
		...
		[ -n "$arlo" ] && iwpriv "$ifname" dni_arlo "${arlo}"
		...
		if [ $vid_len -gt 0 -a $vid_len -eq $br_len ]; then
			...
			while [ $c -le $vid_len ]; do
				...
				brctl hairpin $br $ifname.$vid on
...
pre_qcawifi() {
	...
	case "${action}" in
		...
		enable)
			...
			if [ -n "$ifname" -a "x`/bin/config get enable_arlo_function`" = "x1" ]; then
				brctl show | grep $arlo_br >/dev/null 2>&1
				if [ $? -ne 0 ]; then
					brctl addbr $arlo_br
				fi
				#brctl stp $arlo_br on
				...
				if [ "$product_type" = "Base" ]; then
					...
					if [ "x$udhcpc_process" == "x" ]; then
						udhcpc -b -i brarlo -h RBS50 -N 0.0.0.0 -s /usr/share/udhcpc/default-arlo.script-ext & 
					fi
	...
	case "${action}" in
		enable)
			local icm_enable qwrap_enable lowi_enable
             ...
             [ "x`/bin/config get enable_arlo_function`" = "x1" ] && /usr/sbin/brctl setageing brarlo 65535
    ...
    for device in ${devices}; do
		...
		add_lan_restricted_chain
            ...
            if [ "$lan_restricted" = "1" ]; then
                ...
                config_get ifname "$vif" ifname
                config_get bridge "$vif" bridge
#               conifg_get lan_ipaddr "$vif" lan_ipaddr
		lan_ipaddr=$(ifconfig $bridge | grep "inet addr" | awk '{print $2}'| awk -F ':' '{print $2}')
                if [ "$arlo" != 1 ];then
...
#                config_get lan_ipaddr "$vif" lan_ipaddr
				lan_ipaddr=$(ifconfig $sbridge | grep "inet addr" | awk '{print $2}'| awk -F ':' '{print $2}')
                lan_ipv6addr=$(ifconfig $bridge | grep Scope:Link | awk '{print $3}' | awk -F '/' '{print $1}')
