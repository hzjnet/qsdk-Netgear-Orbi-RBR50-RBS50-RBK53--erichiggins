...
hostapd_set_bss_options() {
	...
	[ -n "$backhaul" ] && append "$var" "backhaul_if=$backhaul" "$N"
	[ -n "$arlo" ] && append "$var" "arlo=1" "$N"  # Netgear Arlo VAP
...
EOF
	...
	if [ $wps_vap_tie_dbdc -ne 0 ]; then
		echo -e "/var/run/hostapd-$ifname.conf \c\h" >> /tmp/hostapd_conf_filename
            ...
            if [ "x$hostapd_debug" != "x" ]; then
                hostapd_global_exist=`ps | grep hostapd | grep -v hostapd_cli | grep -v grep | grep "\-B"`
                if [ -n "$hostapd_global_exist" ]; then
                    killall hostapd
                    sleep 3
                    rm -rf /var/run/hostapd-global.pid
                fi
                hostapd_debug_exist=`ps | grep hostapd | grep -v hostapd_cli | grep -v grep | grep "\-d"`
                if [ -z "$hostapd_debug_exist" ]; then
                    hostapd -g /var/run/hostapd/global -$hostapd_debug -P /var/run/hostapd-global.pid &
                fi
            fi
