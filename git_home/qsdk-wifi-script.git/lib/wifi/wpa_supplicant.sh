...
EOF
	[ -z "$proto" -a "$key_mgmt" != "NONE" ] || {\
		config_get wpa_supplicant_debug_level "$vif" wpa_supplicant_debug_level
		case "$wpa_supplicant_debug_level" in
			1)
				wpa_supplicant_debug="d" ;;
			2)
				wpa_supplicant_debug="dd" ;;
			3)
				wpa_supplicant_debug="ddd" ;;
			4)
				wpa_supplicant_debug="dddd" ;;
			*)
				wpa_supplicant_debug=""
				;;
	        esac
		...
		then
			if [ "x$wpa_supplicant_debug" = "x" ]; then
				...
				if [ -n "$wpa_supplicant_debug_exist" ]; then
		...
		if [ "x$wpa_supplicant_debug" != "x" ]; then
			wpa_supplicant_global_exist=`ps | grep wpa_supplicant | grep -v grep | grep "\-B"`
			if [ -n "$wpa_supplicant_global_exist" ]; then
				killall wpa_supplicant
				sleep 3
			fi
			wpa_supplicant_debug_exist=`ps | grep wpa_supplicant | grep -v grep | grep "\-d"`
			if [ -z "$wpa_supplicant_debug_exist" ]; then
				wpa_supplicant -g /var/run/wpa_supplicantglobal -$wpa_supplicant_debug -P /var/run/wpa_supplicant-global.pid &
			fi
		fi
