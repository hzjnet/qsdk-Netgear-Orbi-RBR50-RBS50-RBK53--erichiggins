...
if [ "${wl2g_backhaul_not_connected}" -a "${wl5g_backhaul_not_connected}" -a "x$cap_mode" != "xCAP" ]; then
    ...
    [ -f /usr/sbin/port_detect.sh ] && /usr/sbin/port_detect.sh check
else
    ...
    soap_auth_block=0
    if [ -f /tmp/last_auth_block ]; then
	[ "`cat /tmp/last_auth_block`" = "1" ] && soap_auth_block=1
    fi
	# Satellite && block down fronthaul ssid
    if [ "x$soap_auth_block" = "x1" ]; then
	[ -f /usr/sbin/port_detect.sh ] && /usr/sbin/port_detect.sh check
    else
