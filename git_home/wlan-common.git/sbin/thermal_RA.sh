...
update_thermalComponentAnalytics(){
    ...
    [ $otpTriggerStatus -ne 0 ] && otpTriggerLongestDuration=$((otpTriggerLongestDuration+$sleep_time))
    update_extremum_temp
    #update otpTriggerCount
    [ $otpTriggerStatus -eq 0 -a $cur_level -ne 0 ] && otpTriggerCount=$((otpTriggerCount+1))
