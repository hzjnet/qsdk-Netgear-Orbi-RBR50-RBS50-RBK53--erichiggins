...
void dni_check_arp_attack_and_calcute_count_log(__be32 sip, __be32 tip)
{
	/* Check arp attack and log this event:
	 * ote that we have not concluded arp attack specific definition.
	 * Assume arp attack action like this:
	 * quickly && almost 10 per second && almost has not gap.
	 */
	if(ctl_arp_attack_protect == 1) {
		if(s_ip == 0 && t_ip == 0) {
			s_ip = sip;
			t_ip = tip;
...
/* Handle changes from /proc interfaces */
static int ipv4_arp_attack_protect(struct ctl_table *table, int write,
								void __user *buffer,
								size_t *lenp, loff_t *ppos)
{
	...
	if (write && ret == 0) {
		if((ctl_arp_attack_protect == 0) && ((int)(val[0] - '0') == 1)) {
			
			/* Enable arp_attack_protect */
			ctl_arp_attack_protect = (int)(val[0] - '0');
			setup_timer(&expire_timer, expire_process, 0);

		} else if((ctl_arp_attack_protect == 1) && ((int)(val[0] - '0') == 0)) {

			/* Disable arp_attack_protect */
			ctl_arp_attack_protect = (int)(val[0] - '0');
...
static struct ctl_table ipv4_table[] = {
	...
	{
		.procname       = "is_satelite",
		.data           = &sysctl_is_satelite,
		.maxlen         = sizeof(int),
		.mode           = 0644,
		.proc_handler   = proc_dointvec,
	},
	{
		.procname       = "block_dhcp",
		.data           = &sysctl_block_dhcp,
		.maxlen         = sizeof(int),
		.mode           = 0644,
		.proc_handler   = proc_dointvec,
	},
    {
       .procname       = "2gbhapifname",
       .data           = &sysctl_2gbhapifname,
       .maxlen         = 16,
       .mode           = 0644,
       .proc_handler   = proc_dostring,
   },
   {
       .procname       = "2gbhstaifname",
       .data           = &sysctl_2gbhstaifname,
       .maxlen         = 16,
       .mode           = 0644,
       .proc_handler   = proc_dostring,
   },
   {
       .procname       = "5gbhapifname",
       .data           = &sysctl_5gbhapifname,
       .maxlen         = 16,
       .mode           = 0644,
       .proc_handler   = proc_dostring,
   },
   {
       .procname       = "5gbhstaifname",
       .data           = &sysctl_5gbhstaifname,
       .maxlen         = 16,
       .mode           = 0644,
       .proc_handler   = proc_dostring,
   },
	...
	{
		.procname       = "stavap_packet",
		.data           = &sysctl_stavap_packet,
		.maxlen         = sizeof(int),
		.mode           = 0644,
		.proc_handler   = proc_dointvec,
	},
...
static struct ctl_table ipv4_net_table[] = {
	...
	},
	{
	...
	},
        {
                ...
                .maxlen         = sizeof(int),
                .mode           = 0644,
                .proc_handler   = ipv4_arp_attack_protect,
        },
	{ }
};
