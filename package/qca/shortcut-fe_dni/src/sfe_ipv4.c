			     ...
			     unsigned int len, struct sfe_ipv4_ip_hdr *iph, unsigned int ihl, bool flush_on_find)
{
	...
	dest_port = udph->dest;
	/* For NTGR L2TP passthrough function and readyshare function */
	if (ntohs(dest_port) == 1701 || ntohs(dest_port) == 137 || ntohs(src_port) == 137){
		spin_lock_bh(&si->lock);
			     ...
			     unsigned int len, struct sfe_ipv4_ip_hdr *iph, unsigned int ihl, bool flush_on_find)
{
	...
	dest_port = tcph->dest;
	/* For NTGR block site function*/
	if (ntohs(dest_port) == 80 || ntohs(dest_port) == 119){
		spin_lock_bh(&si->lock);
		si->exception_events[SFE_IPV4_EXCEPTION_EVENT_UNHANDLED_PROTOCOL]++;
		si->packets_not_forwarded++;
		spin_unlock_bh(&si->lock);
