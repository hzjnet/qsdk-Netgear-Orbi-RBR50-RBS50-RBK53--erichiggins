...
#define USB_SMB_NAME	"NETGEAR EX6200v2"
...
static void add_smbd_share_info(FILE *fp, char *displayname, char *reader, char *writer, char *path, int share_floder_num)
{
	...
	char writelist[256] = {0}, readlist[256] = {0}, invalidlist[256] = {0}, temp[64] = {0};
	...
	if((strcmp(readycloud_enable_info,"1") != 0) || (readycloud_access_info[0] == '\0')){
		...
		if (!strcmp(config_get("usb_passwdNet"), "0")) {
            strncpy(temp, config_get("http_username"), sizeof(temp));
            if (strcmp(temp, "admin")) {
                sprintf(writelist + strlen(writelist), "%s ", temp);
            }
